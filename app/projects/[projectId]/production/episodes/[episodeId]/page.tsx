...
export default async function ProductionEpisodePage({
  ...
  searchParams: Promise<{ scene?: string; error?: string }>
}) {
  ...
  return <StoryboardWorkspace projectId={projectId} episode={episode} scenes={scenes} selectedScene={selectedScene} readiness={readiness} sceneCharacters={sceneCharacters.filter(item => item.sceneId === selectedScene.id)} costumes={costumes} locations={locations} shotAssignments={shotAssignments} issues={issues} jobs={jobs} archivedShots={archivedShots} error={query.error} />
}
